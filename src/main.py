...
def init_input():
    try:
        timestamps = subprocess.check_output(
            ["./times.sh", "input.ass"], text=True)
        dialogues = timestamps.splitlines()
        return dialogues
    except subprocess.CalledProcessError as e:
        print(f"Error while executing times.sh: {e}")
        return []
...
def get_lyrics(dialogues):
    ...
    for i, dialogue in enumerate(dialogues):
        ...
        if n > 0 and lyrics[n - 1] == modified_content:
            continue
...
def transform_result(dialogues, lyrics):
    ...
    for i, dialogue in enumerate(dialogues):
        ...
        previous_dialogue += lyrics[lyrics_pointer -
                                    2] if lyrics_pointer > 1 else ',...'

        current_dialogue = transform_input(dialogue, 'Default')

        next_dialogue = transform_input(dialogue[0:9], 'F')
        next_dialogue += lyrics[lyrics_pointer] if lyrics_pointer < len(
            lyrics) else ',...'
